...
/*        
        for (int x = 0; x < 8; x++) {
            ...
            if (x < 4)
                ...
                t1 = wall2;
            count = 0;
            if ( t1+(diff*2) > 0 && t1+(diff*2) < rows*cols) {
                for (int i = 0; i < 2; i++) {
                    if ( [[wallList objectForKey:[NSNumber numberWithInt:t1]] 
                          containsObject:[NSNumber numberWithInt:(t1+diff)]] ) {
                        count++;
...
-(Boolean) createMaze2
{
    ...
    NSMutableArray *tmpArray = [[NSMutableArray alloc] initWithArray:[fullBreakdownOptionsList allKeys]];
    int max = [tmpArray count];
    ...
    for (int i=0; i < max; i++) {
        ...
        [tmpArray exchangeObjectAtIndex:i withObjectAtIndex:randNum];
    }
    ...
    for (int i = 0; i < max; i++) {
        ...
        num1 = [[fullBreakdownOptionsList objectForKey:[tmpArray objectAtIndex:i]] chosenBlock];
        num2 = [[fullBreakdownOptionsList objectForKey:[tmpArray objectAtIndex:i]] breakdownBlock];
